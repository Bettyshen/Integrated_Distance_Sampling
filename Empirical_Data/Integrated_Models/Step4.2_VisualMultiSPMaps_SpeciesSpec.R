#---
#title: "Visualize Multi-species Prediction Maps (Restricted + non-restircted) with species-specific key function IDS model with capping values for continuous variables"
#author: "Betty Shen"
#date: "2026-01-08" 
#output: html_document
#---
#Goal: the goal of the script is to predict density of multiple speciesin 2017 using species-specific key function IDS model with capping values for continuous variables and visualize the results
...
# This script is aimed for species-specific key function IDS model with capping values for continuous variables and fix distance 500 m as truncation distance
...
# All species list
species_list <- read.csv("/nfs/stak/users/shenf/hpc-share/IntegrationModel/Data/species_include.csv")
# Get species that should be included (Include == "Yes")
species_to_include <- species_list$Species[species_list$Include == "Yes"]
...
print(restricted_species)
 ...
 # Filter restricted species to only include those that should be included (Include == "Yes")
restricted_species_to_include <- restricted_species[restricted_species %in% species_to_include]
cat("Found", length(restricted_species_to_include), "restricted species to process:\n")
print(restricted_species_to_include)
...
# Path to prediction raster (species-specific detection models)
raster_dir <- "/nfs/stak/users/shenf/hpc-share/IntegrationModel/PredictionMaps/ContinuVars/FixDistance/SpeciesSpecif"
...
for (sp in restricted_species_to_include) {
  cat("Processing", sp, "...\n")
...
# ===============Plot and save PNG======================= #
  ...
  png_file <- file.path("/nfs/stak/users/shenf/hpc-share/IntegrationModel/Raster/PredictionMap/ContinVars/FixDistance/SpeciesSpecific", 
...
non_restricted_species <- species_to_include[!species_to_include %in% restricted_species_to_include]
cat("Found", length(non_restricted_species), "non-restricted species to process:\n")
print(non_restricted_species)


# Process and visualize each species in one efficient loop

for (sp in non_restricted_species) {
  ...
  cat("Preds dimensions:", dim(preds), "\n")
  cat("Raster dimensions:", dim(raster), "\n")
  ...
  if ("x" %in% names(preds) && "y" %in% names(preds) && "density_pred" %in% names(preds)) {
    ...
    if (nrow(preds) != nrow(raster)) {
      cat("Warning: Prediction data has", nrow(preds), "rows but raster has", nrow(raster), "rows.\n")
      cat("Cannot merge. Skipping", sp, "due to dimension mismatch.\n")
      next
    }
    ...
    if ("density_pred" %in% names(raster_out)) {
      ...
      if (is.na(pred_col)) {
        cat("Warning: Could not find prediction column in data. Skipping", sp, ".\n")
        next
      }
    }
  }
  ...
  if (min(raster_out$x) >= -180 && max(raster_out$x) <= 180 && 
      min(raster_out$y) >= -90 && max(raster_out$y) <= 90) {
    cat("Coordinates appear to be in WGS84. Setting CRS and projecting to Oregon State Plane...\n")
    crs(pred_rast) <- "EPSG:4326"
    # Always project to target CRS (Oregon State Plane EPSG:2992)
    pred_rast <- project(pred_rast, target_crs)
    cat("Projection to Oregon State Plane successful.\n")
  } else {
    cat("Coordinates appear to be already projected. Setting CRS to target CRS.\n")
    crs(pred_rast) <- target_crs
  }
