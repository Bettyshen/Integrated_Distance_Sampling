# Goal: Test if half-normal and species-specific detection functions predicted median abundance are significantly different
# Date: 2026-01-16
# Author: Betty Shen
# 
...
data.cols <- data.select %>%
    select(Common_Name, HalfNorm_Median, SpeciesSpec_Median)
str(data.cols)

# Convert data into long format: species, model type (half-normal or species-specific), and abundance
data_long <- data.cols %>%
    pivot_longer(
        cols = c(HalfNorm_Median, SpeciesSpec_Median),
        names_to = "Model_Type",
        values_to = "Abundance"
    ) %>%
    mutate(
        Model_Type = case_when(
            Model_Type == "HalfNorm_Median" ~ "Half-normal",
            Model_Type == "SpeciesSpec_Median" ~ "Species-specific",
            TRUE ~ Model_Type
        )
    )
str(data_long)
...
# Save MCMC output
saveRDS(mcmc.output, file = "/Volumes/T7/Chapter2/DataIntegration/Results/Bayes/LinearMixedModel_MedianAbund_output.rds")

# Read MCMC output
mcmc.output <- readRDS("/Volumes/T7/Chapter2/DataIntegration/Results/Bayes/LinearMixedModel_MedianAbund_output.rds")
attach.nimble(mcmc.output$samples)
...
beta1_results <- beta1_results[order(beta1_results$Median), ]
...
# Export beta1_results to csv
write.csv(beta1_results, file = "/Volumes/T7/Chapter2/DataIntegration/Results/Bayes/LinearMixedModel_MedianAbund_results.csv", row.names = FALSE)
...
if (nrow(significant_species) > 0) {
    print(significant_species[, c("Species", "Lower_2.5", "Median", "Upper_97.5")])
} else {
...
create_effect_plot <- function(data, title_suffix) {
    p <- ggplot(data, aes(x = Median, y = Species)) +
        ...
        # Add median point
        geom_point(aes(x = Median), size = 2, color = "orange", fill = "orange", 
                  shape = 21, stroke = 1) +
...
ggsave(filename = paste0(plot_dir, "Beta1_MedianEffectPlot_Part1.png"), 
       plot = plot1, width = 10, height = 12, dpi = 300)
cat("Saved: Beta1_MedianEffectPlot_Part1.png\n")

ggsave(filename = paste0(plot_dir, "Beta1_MedianEffectPlot_Part2.png"), 
       plot = plot2, width = 10, height = 12, dpi = 300)
cat("Saved: Beta1_MedianEffectPlot_Part2.png\n")
