# Abundance Comparison Simulation (equal availability) with AIC model selection
# Date: 2026-07-01
# Author: Betty Shen
# Correct vs incorrect detection functions in distsamp; min-AIC key-function choice.
...
keyfun_labels <- c("half-normal", "hazard-rate", "negative-exponential")

# Initialize results storage
results <- data.frame(
  pooled_replicate = integer(),
  true_detection_function = character(),
  AIC_selected_function = character(),
  AIC_selected = logical(),
  AIC_half = numeric(),
  AIC_haz = numeric(),
  AIC_exp = numeric(),
  n_sites = integer(),
...
# backTransform is from unmarked package and convert individuals units to per km^2
  ...
  aic_selected_function <- keyfun_labels[which.min(aic_vals)]
  aic_selected <- identical(aic_selected_function, true_detection_label)

  data.frame(
    pooled_replicate = pooled_replicate,
    true_detection_function = true_detection_label,
    AIC_selected_function = aic_selected_function,
    AIC_selected = aic_selected,
    AIC_half = aic_vals["half"],
    AIC_haz = aic_vals["hazard"],
    AIC_exp = aic_vals["exp"],
...
na_result_row <- function(pooled_replicate, tau, mean_abundance, true_detection_label) {
  data.frame(
    pooled_replicate = pooled_replicate,
    true_detection_function = true_detection_label,
    AIC_selected_function = NA_character_,
    AIC_selected = NA,
    AIC_half = NA, AIC_haz = NA, AIC_exp = NA,
...
cat("\nAIC selection rate by tau and true detection function:\n")
aic_selection_summary <- results %>%
  group_by(tau, true_detection_function) %>%
  summarise(
    n_replicates = n(),
    n_aic_correct = sum(AIC_selected, na.rm = TRUE),
    prop_aic_correct = round(mean(AIC_selected, na.rm = TRUE), 3),
    .groups = "drop"
  )
print(aic_selection_summary)

cat("\nAIC-selected function counts (true detection × selected):\n")
print(table(
  True = results$true_detection_function,
  AIC_selected = results$AIC_selected_function,
  useNA = "ifany"
))

# Verify AIC_selected_function matches the minimum of the three reported AIC values
results$AIC_min_model_check <- apply(
  cbind(results$AIC_half, results$AIC_haz, results$AIC_exp),
  1,
  function(x) {
    if (anyNA(x)) {
      return(NA_character_)
    }
    keyfun_labels[which.min(x)]
  }
)

cat("\nAIC selection consistency check:\n")
cat("Rows with complete AIC values:", sum(complete.cases(results[, c("AIC_half", "AIC_haz", "AIC_exp")])), "\n")
n_mismatch <- sum(
  results$AIC_selected_function != results$AIC_min_model_check,
  na.rm = TRUE
)
cat("Rows where AIC_selected_function != min(AIC_half, AIC_haz, AIC_exp):", n_mismatch, "\n")

if (n_mismatch > 0) {
  cat("\nMismatched rows (first 10):\n")
  mismatch_idx <- which(results$AIC_selected_function != results$AIC_min_model_check)
  print(head(results[mismatch_idx, c(
    "pooled_replicate", "true_detection_function", "tau",
    "AIC_selected_function", "AIC_min_model_check",
    "AIC_half", "AIC_haz", "AIC_exp"
  )], 10))
}

cat("\nMean AIC by tau and true detection function:\n")
aic_summary <- results %>%
  group_by(tau, true_detection_function) %>%
  summarise(
    mean_AIC_half = round(mean(AIC_half, na.rm = TRUE), 2),
    mean_AIC_haz = round(mean(AIC_haz, na.rm = TRUE), 2),
    mean_AIC_exp = round(mean(AIC_exp, na.rm = TRUE), 2),
    .groups = "drop"
  )
print(aic_summary)
...
# Export results to CSV (drop internal verification columns)
output_file <- "/Volumes/T7/Chapter2/DataIntegration/Results/DataSimulation/Revision/abundance_simulation_results_AIC.csv"
write.csv(
  results[, !names(results) %in% c("AIC_min_model_check")],
  file = output_file,
  row.names = FALSE
)
...
perceptability_file <- "/Volumes/T7/Chapter2/DataIntegration/Results/DataSimulation/Revision/perceptability_summary_AIC.csv"
...
rel_bias_file <- "/Volumes/T7/Chapter2/DataIntegration/Results/DataSimulation/Revision/rel_bias_medians_AIC.csv"
