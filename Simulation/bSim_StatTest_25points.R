...
#       (3) Export results to CSV aligned with figure (3 rows × 4 columns)
#
...
data <- read.csv("/Volumes/T7/Chapter2/DataIntegration/Results/DataSimulation/PointCounts25/density_simulation_results_25counts.csv")
colnames(data)
# Reshape to long: one row per (simulation, tau, true_detection_function, fitted_model) with relative_bias
data_long <- bind_rows(
  data %>%
    transmute(simulation, tau, true_detection_function,
...
# ============================================================================
# Median of relative bias by scenario (aligned with figure: 12 panels)
# ============================================================================
...
cat("Figure layout: 3 rows (known detection function) × 4 columns (tau). HN/HR/NE = fitted model.\n\n")
print(median_wide)
cat("\n")

output_dir <- "/Volumes/T7/Chapter2/DataIntegration/Results/DataSimulation/PointCounts25"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
write.csv(median_wide, file.path(output_dir, "median_rel_bias_compare_25counts.csv"), row.names = FALSE)
cat("Median by scenario saved to: median_rel_bias_compare_25counts.csv\n\n")
...
output_dir <- "/Volumes/T7/Chapter2/DataIntegration/Results/DataSimulation/PointCounts25"
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
write.csv(global_assump, file.path(output_dir, "global_assumption_check_compare_25counts.csv"), row.names = FALSE)
cat("Global assumption check saved to:", file.path(output_dir, "global_assumption_check_compare_25counts.csv"), "\n\n")
...
# Figure layout: 3 rows (true detection function) × 4 columns (tau)
# Row 1: half-normal | Row 2: hazard-rate | Row 3: negative-exponential
# Col 1: tau=1 | Col 2: tau=2 | Col 3: tau=3 | Col 4: tau=4
...
results_export <- results_df %>%
  select(tau, true_detection_function,
...
# ============================================================================
# Export to CSV (aligned with figure: 12 rows, row order = panel order)
# ============================================================================
output_dir <- "/Volumes/T7/Chapter2/DataIntegration/Results/DataSimulation/PointCounts25"
output_csv <- file.path(output_dir, "stat_test_results_compare_25counts.csv")
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
write.csv(results_export, file = output_csv, row.names = FALSE)

cat("=== STATISTICAL TEST RESULTS (per panel) ===\n\n")
cat("Figure layout: 3 rows (known detection function) × 4 columns (tau)\n")
...
write.csv(perceptability_summary, file.path(output_dir, "perceptability_summary_25counts.csv"), row.names = FALSE)
cat("Perceptability summary saved to:", file.path(output_dir, "perceptability_summary_25counts.csv"), "\n\n")
